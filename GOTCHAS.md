...
- **EBA action:** **Deferred to Spec 4 (hardening) — do NOT smuggle into the ingestion slice.** The real build must make ingestion incremental + async: the upload writes to the inbox and enqueues a job; a worker ingests ONLY the new file via `INSERT … ON CONFLICT (idempotency_key) DO UPDATE` (append/upsert, never TRUNCATE), behind a per-source lock. That single change removes the O(history) rebuild, the in-request block, the empty-dashboard window, and most of the file-race surface at once. The 25 MiB synchronous cap is a stopgap that keeps the demo honest about its ceiling, not a substitute.
